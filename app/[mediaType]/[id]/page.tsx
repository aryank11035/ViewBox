...
export default async function ShowMedia({params} : Params) {
    ...
    return (
        <section className="max-w-[1800px] min-h-screen mx-auto  border-l border-r border-white/10 relative bg-[#111111] backdrop-blur-3xl pt-20 ">
            <div className = 'w-full  relative md:pt-0 pt-10 md:px-0 px-2'>    
                <div className="inset-0 w-full max-h-[250px] relative hidden md:block ">
                   
                        <img
                            src={mediaData.backdrop_path ? `https://image.tmdb.org/t/p/w500${mediaData.backdrop_path}` : '/placeholder-movie.jpg'}
                            alt={mediaData.title}
                            className=" inset-0 w-full max-h-[250px] object-cover "
                        />
                    
                    <div className="absolute inset-0 bg-black/50 backdrop-blur-sm"></div>
                </div>   
                <div className="mx-auto w-full relative md:top-0 md:-translate-y-1/7 md:translate-x-0 md:px-10 flex justify-between md:flex-row flex-col gap-10 ">                    
                    <div className=" w-full md:max-w-[400px] max-w-[310px] aspect-[2/3] z-10 rounded-sm mx-auto mb-25 md:mb-20">
                    ...
                    <div className="w-full   flex flex-col gap-7 mt-3 md:mt-0">
                        <div className="w-full  h-fit flex flex-col md:flex-row md:justify-end md:px-0 px-6 md:gap-15 md:pt-2 " >
                            <div className="text-xl font-medium text-white/50 md:text-right flex gap-1 md:flex-col flex-row-reverse  justify-between ">
                                <p className="md:text-4xl text-xl text-white">{mediaType === 'movie' ? 'Movie' : 'Tv Series'}</p>
                                <h1>Type</h1>
                            </div>
                            <div className="text-xl font-medium text-white/50 md:text-right flex gap-1 md:flex-col flex-row-reverse  justify-between ">
                                <p className="md:text-4xl text-xl text-white">{mediaData.adult ? '+18' : '>18'}</p>
                                <h1>Age</h1>
                            </div>
                            <div className="text-xl font-medium text-white/50 md:text-right flex gap-1 md:flex-col flex-row-reverse  justify-between ">
                                <p className="md:text-4xl text-xl text-white">{mediaData.original_language.toUpperCase()}</p>
                                <h1>Language</h1>
                            </div>
                            <div className="text-xl font-medium text-white/50 md:text-right flex gap-1 md:flex-col  flex-row-reverse justify-between">
                                <p className="md:text-4xl text-xl text-white">{mediaData.vote_average.toFixed(1)}/10</p>
                                <h1>Rating</h1>
                            </div>
                           {mediaData.runtime ?  <div className="text-xl font-medium text-white/50 md:text-right flex gap-1 md:flex-col  flex-row-reverse justify-between ">
                                <p className="md:text-4xl text-xl text-white">{mediaData.runtime}mins</p>
                                <h1>Runtime</h1>
                            </div> : ''}
                        </div>
                        <div className="w-full  font-bold px-2">
                           ...
                           <div className="flex flex-col md:flex-row gap-4 mt-5">
                                ...
                                <div className="relative w-full md:w-fit h-fit mt-2 flex-wrap">
                                    <div className="hidden md:block
                                                    absolute inset-0 
                                                    border border-[rgba(255,255,255,0.2)]
                                                    bg-[repeating-linear-gradient(45deg,rgba(255,255,255,0.1)_0,rgba(255,255,255,0.1)_1px,transparent_0,transparent_50%)]
                                    ...
                                    <div className="cursor-pointer relative transform  md:text-left md:text-xl text-xl font-bold font-sans px-6 py-2 border border-green-500 bg-green-500 text-green-900 text-center w-full md:w-fit md:translate-x-2 md:-translate-y-2 hover:z-10 transition-all duration-300 hover:translate-x-0 hover:translate-y-0  active:translate-x-0 active:-translate-y-0 active:bg-transparent active:text-white active:border-[rgba(0,0,0,0.2)]">
                                            Add to Watchlist      
                                    </div>
                                </div>
                                ...
                                <div className="relative w-full md:w-fit h-fit mt-2 flex-wrap">
                                    ...
                                    <div className="cursor-pointer relative transform  md:text-left md:text-xl text-xl font-bold font-sans px-6 py-2 border border-red-500 bg-red-500 text-red-900 text-center w-full md:w-fit md:translate-x-2 md:-translate-y-2 hover:z-10 transition-all duration-300 hover:translate-x-0 hover:translate-y-0  active:translate-x-0 active:-translate-y-0 active:bg-transparent active:text-white active:border-[rgba(0,0,0,0.2)]">
