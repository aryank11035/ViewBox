...
export default function HeaderSearchBar({onSearch , cancelButton , searchString , cancelSearch} : HeaderSearchBarProps){
    ...
    return (
        <>
            ...
            <Input className="absolute inset-0 h-full border border-[rgba(255,255,255,0.2)] rounded-xs outline-0 pl-10 text-white" placeholder="Search Movies...(Ctrl+k)" onChange={(e) =>handleSearch(e.target.value)} value={searchString} ref={inputRef}/>
            {
                cancelButton && (
